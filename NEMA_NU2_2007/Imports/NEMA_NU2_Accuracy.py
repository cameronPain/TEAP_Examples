...
class Accuracy:
    ...
    def generate_report(self, save_directory = '', file_name = 'NEMA_NU2_Accuracy', system_details = ['Siemens', 'Intevo'], captions = [''], delete_tex_files = True):
        #Generate Plots
        fontsize = 26
        fig, (ax0,ax1) = pyplot.subplots(2,1, figsize=(21,22))
        ...
        fig.text(0.07, 0.65, 'Event rate ($s^{-1}$)', fontsize=fontsize, rotation='vertical')
        fig.text(0.07, 0.20, 'Dead time percentage', fontsize=fontsize, rotation='vertical')
    ...
    def __get_circle_ROI_mask(self, radius = 90): #90 mm
        ...
        for i in range(recon_volume_dims[-2]):
    ...
    def analyse_data(self):
        ...
        for line in deadtime_percent:
            min_vals.append(n.amin(line))
            max_vals.append(n.amax(line))
...
def main(root_dir):
    ...
    accuracy.generate_report(captions = ['Dead time losses of the system as a function of effective activity concentration.'], delete_tex_files = True)

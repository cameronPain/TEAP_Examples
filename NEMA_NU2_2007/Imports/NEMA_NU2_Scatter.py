...
class Scatter:
    ...
    def generate_report(self, save_directory = '', file_name = 'NEMA_NU2_CountRate_Scatter', system_details = ['Siemens', 'Intevo'], captions = ['',''], delete_tex_files = True):
        #Generate Plots
        fontsize = 22
        fig, (ax0,ax1) = pyplot.subplots(2,1, figsize=(21,16))
        ...
        fig.text(0.07, 0.4, 'Event rate ($ms^{-1}$)', fontsize=fontsize, rotation='vertical')
        fig.text(0.42, 0.05, 'Activity concentration ($kBq/ml$)', fontsize=fontsize)
        self.count_rate_plot = fig
        
        scatter_fig, ax2 = pyplot.subplots(1,1,figsize=(21,9))
        ...
        self.scatter_fraction_plot = scatter_fig
        ...
        with pylatex_document.create(Figure(position = '!t')) as figure:
            pyplot.figure(self.scatter_fraction_plot.number)
            figure.add_plot(width = NoEscape(r"\textwidth"))
    ...
    def __get_peak_and_scatter(self, image, random, x_pixel_spacing):
        ...
        for i in range(len(image)):
            ...
            left_val_0            = row[centre_px - int(slices_for_bound) - 1]
            left_val_1            = row[centre_px - int(slices_for_bound)]
            right_val_0           = row[centre_px + int(slices_for_bound)]
            right_val_1           = row[centre_px + int(slices_for_bound) + 1]
            CR_l                  = ((left_val_1 - left_val_0)*( 1 - (slices_for_bound - int(slices_for_bound))) + left_val_0)
            CR_r                  = ((right_val_1 - right_val_0)*(slices_for_bound - int(slices_for_bound)) + right_val_0)
            
            peak_scatter_random_ave    = ((CR_l + CR_r)/2.0)
            peak_pixels_scatter_random = 2*slices_for_bound
            peak_scatter_random        = peak_scatter_random_ave*peak_pixels_scatter_random
            off_peak_scatter_random    = n.sum(row[:centre_px - int(slices_for_bound)]) + n.sum(row[centre_px + int(slices_for_bound) + 1:])
            scatter_randoms            = peak_scatter_random + off_peak_scatter_random
            true_counts                = n.sum([row[centre_px-int(slices_for_bound):centre_px+int(slices_for_bound)]]) - peak_scatter_random
        ...
        return true, scatter, random, NEC, total, scatter_fraction
    ...
    def analyse_data(self):
        ...
        for i in range(len(concat_prompts)):
            ...
            projection_image = n.sum(aligned_sinogram, axis = 1) # Project the aligned sinogram along the angle axis. This gives you a bunch of
            ...
            true_counts, scatter_counts, random_counts, NE_counts, total_counts, scatter_fraction =  self.__get_peak_and_scatter(projection_image, projection_rand, self.pixel_spacings[i][2]) #Calculate the relevant quantities from the aligned sinogram.
        ...
        self.System_Scatter_Fraction = n.sum(ScatterC)/(n.sum(TotalC) - n.sum(RandomC))
...
def main(root_dir):
    ...
    scatter.generate_report(captions = ['Count rate as a function of activity concentration.','Scatter fraction as a function of activity concentration with the system scatter fraction plotted in red for comparison.'], delete_tex_files = False)
